 ...
 */
export default function preProcessImg(image: cv.Mat, options: Options): ImagePreprocessResult {
...
function getGroup(image: cv.Mat): number[][] {
  const gray = image.cvtColor(cv.COLOR_BGR2GRAY);
  const thresh = gray.threshold(0, 255, cv.THRESH_BINARY_INV + cv.THRESH_OTSU);
  // 开运算, 去除图像中的小亮点、噪声
  const kernel = new cv.Mat(3, 3, cv.CV_8UC1, 1);
  const opening = thresh.morphologyEx(kernel, cv.MORPH_OPEN, undefined, 2);
  // 确定背景区域
  const background = opening.dilate(kernel, undefined, 3);
  // 寻找前景区域
  ...
  for (let i = 0; i < markersData.length; i++) {
    for (let j = 0; j < markersData[i].length; j++) {
      if (unknownData[i][j] === 255) markersData[i][j] = 0;
    }
  }
  // 转换矩阵格式
...
function getDistAndContour(markers: number[][]): [[number, number, number][][], Point[][], number[]] {
  // let markers = JSON.parse(JSON.stringify(groupData))
  // 获取数据内所有的组，去除背景1
  const labels = _.uniq(_.flatten(markers)).splice(1);
  const distData: [number, number, number][][] = [];
  const contourData: Point[][] = [];
  ...
  labels.forEach((label) => {
    ...
    contourData.push([]);
    let area = 0;
    for (const i in contour) {
      area += Math.floor(contour[i].area);
      for (const j of contour[i].getPoints()) {
...
function cutImage(image: cv.Mat, options: Options) {
  ...
  const startRow = Math.floor((options.height - height) / 2);
  const startCol = Math.floor((options.width - width) / 2);
  for (let row = startRow, iRow = 0; iRow < height; row++, iRow++) {
    for (let col = startCol, iCol = 0; iCol < width; col++, iCol++) {
